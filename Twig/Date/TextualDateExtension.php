...
class TextualDateExtension extends \Twig_Extension
{
    ...
    {
        return [
            'textualDate'    => new \Twig_SimpleFilter('textualDate', [$this, 'textualDateFilter']),
        ];
    }
    ...
    public function textualDateFilter($date)
    {
        ...
        return $this->translator->transChoice($translationString, $diffUnit['value'], ['%value%' => $diffUnit['value']], 'date');
    }
    ...
    protected function getHighestDiffUnitAndValue($diff)
    {
        ...
        foreach ($properties as $prop) {
            if ($diff->$prop > 0) {
                return ['unit' => $prop, 'value' => $diff->$prop];
            }
        }
